...
@Component({
  ...
  styleUrl: './public-monitor-detail.scss',
})
export class PublicMonitorDetailPage {
  ...
  statusMessage(monitor: PublicStatusMonitor): string {
    if (!monitor.is_active) return `${monitor.name} monitoring is paused`;
    if (monitor.status === 'up') return `${monitor.name} is operational`;
    if (monitor.status === 'down') return `${monitor.name} is currently unavailable`;
    return `${monitor.name} has not reported a status yet`;
  }
  ...
  eventTitle(event: PublicMonitorEvent, monitorName: string): string {
    if (event.event_type === 'created') return `${monitorName} was created`;
    if (event.event_type === 'down') return `${monitorName} went down`;
    return `${monitorName} went up again`;
  }

  eventDuration(event: PublicMonitorEvent): number | null {
    if (event.duration_seconds === null) return null;
    if (!event.ongoing) return event.duration_seconds;
  ...
  private connect(): void {
    ...
    this.source = new EventSource(
      `/api/status-pages/public/${encodeURIComponent(this.slug)}/monitors/${encodeURIComponent(this.monitorId)}/events`,
    );
    this.source.onopen = () => {
    ...
    this.source.addEventListener('snapshot', (event) => {
      try {
        ...
        this.detail.set(
          current && currentEventIds === incomingEventIds
