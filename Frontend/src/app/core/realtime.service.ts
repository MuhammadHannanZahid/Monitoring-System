...
export class RealtimeService {
  ...
  liveUptimePercentage(overview: MonitorOverview): number | null {
    if (overview.uptime_percentage === null) return null;
    ...
    if (measurementSeconds <= 0) return overview.uptime_percentage;
  ...
  liveIncidentDuration(incident: DashboardIncident): number {
    if (incident.resolved_at) return incident.duration_seconds ?? 0;
    const now = this.now();
