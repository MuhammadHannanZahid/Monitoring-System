...
from app.shared.models.auth_profile import AuthProfileResponse, CreateAuthProfileRequest, UpdateAuthProfileRequest
...
async def create_profile(request: CreateAuthProfileRequest, service: AuthProfileService = Depends(get_auth_profile_service)):
    try:
...
async def list_profiles(service: AuthProfileService = Depends(get_auth_profile_service)):
...
async def get_profile(profile_id: str, service: AuthProfileService = Depends(get_auth_profile_service)):
...
async def update_profile(profile_id: str, request: UpdateAuthProfileRequest, service: AuthProfileService = Depends(get_auth_profile_service)):
    try:
...
async def delete_profile(profile_id: str, service: AuthProfileService = Depends(get_auth_profile_service)):
