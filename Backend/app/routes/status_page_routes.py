...
def get_status_page_service(
    ...
    monitor_service: MonitorManager = Depends(get_monitor_service),
    dashboard_service: DashboardManager = Depends(get_dashboard_service),
) -> StatusPageManager:
...
async def get_public_page(
    slug: str,
    service: StatusPageManager = Depends(get_status_page_service),
):
...
async def get_public_monitor_detail(
    slug: str,
    monitor_id: str,
    service: StatusPageManager = Depends(get_status_page_service),
):
...
async def stream_public_monitor_detail(
    slug: str,
    monitor_id: str,
    request: Request,
    service: StatusPageManager = Depends(get_status_page_service),
        ...
        try:
            ...
            while not await request.is_disconnected():
                ...
                if status_page_changed:
                    updated_page = await service.get_page_model(page.id)
