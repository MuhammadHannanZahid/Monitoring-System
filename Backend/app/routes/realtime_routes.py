...
async def stream_events(
    request: Request,
    response: Response,
    current_user: CurrentUserResponse = Depends(require_viewer()),
):
            ...
            while not await request.is_disconnected():
                ...
                try:
                    update: RealtimeUpdate = await asyncio.wait_for(
                        queue.get(),
                        timeout=min(KEEP_ALIVE_SECONDS, remaining),
                    )
                except TimeoutError:
                ...
                if any(
                    kind == "user" and entity_id == current_user.id
                    for kind, entity_id in update.changed
                ):
                    return
                snapshot = (
                    update.admin_snapshot if is_admin else update.common_snapshot
                )
