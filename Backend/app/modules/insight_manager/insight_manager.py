...
from app.service.mongo_db.shared_models.db_insight_model import (DashboardSummaryResponse, DashboardIncidentResponse,
    DashboardActivityResponse, MonitorDetailResponse, MonitorIncidentResponse, MonitorOverviewResponse,
    ResponseHistoryResponse, ResponseHistoryPoint, UptimeResponse, StatusHistoryResponse, StatusHistoryPoint)
...
class DashboardManager:
    ...
    async def get_summary(self) -> DashboardSummaryResponse:
        ...
        uptime_values = [
            ...
            if overview.is_active and overview.uptime_percentage is not None
        ]
        overall_uptime = (
    ...
    async def get_monitor_overviews(self) -> list[MonitorOverviewResponse]:
        ...
        first_check_times, incidents_by_monitor = await asyncio.gather(
        ...
        for monitor in monitors:
            ...
            if monitor.status != MonitorStatus.UNKNOWN and first_check_at is not None:
                ...
                downtime_seconds = sum(
                    self._incident_overlap_seconds(
                        incident.started_at,
                ...
                uptime_percentage = (
                    round(max(0.0, 1 - downtime_seconds / measured_seconds) * 100, 2)
            ...
            if monitor.status == MonitorStatus.UP:
                uptime_started_at = (
                    latest_incident.resolved_at
                    if latest_incident is not None and latest_incident.resolved_at is not None
                    else first_check_at
                )
                if uptime_started_at is not None:
                    current_uptime_seconds = self._duration_seconds(
                        uptime_started_at,
                        measurement_end,
                    )

            latest_downtime_seconds = (
                latest_incident.duration_seconds if latest_incident is not None else 0
            )
            overviews.append(
    ...
    async def get_monitor_detail(self, monitor_id: str) -> MonitorDetailResponse:
        overview = next(
            (
                item
                for item in await self.get_monitor_overviews()
        ...
        incidents = (await self.incident_service.get_for_monitors([monitor_id])).get(
            monitor_id,
            [],
        )
    ...
    def _incident_overlap_seconds(
        ...
        overlap_end = min(
            DashboardManager._as_utc(resolved_at) if resolved_at is not None else period_end,
            period_end,
        )
